...
from database.models import MissionUser # Adjust import path if necessary
...
def deleteMissionUserByMissionUserId(
    missionUserId: int = Path(..., description="ID of the MissionUser record to delete"),
    db: Session = Depends(get_db)
) -> Dict[str, str]:
    db_mission_user = db.get(MissionUser, missionUserId)
    if db_mission_user is None:
        raise NotFoundException(detail=f"MissionUser record with ID {missionUserId} not found")
    try:
        db.delete(db_mission_user)
        db.commit()
        return {"message": f"MissionUser record with ID {missionUserId} deleted successfully"}
