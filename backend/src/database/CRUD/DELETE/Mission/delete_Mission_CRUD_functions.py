...
from database.models import Mission # Adjust import path if necessary
...
def deleteMissionByMissionId(
    missionId: int = Path(..., description="ID of the mission to delete"),
    db: Session = Depends(get_db)
) -> Dict[str, str]:
    db_mission = db.get(Mission, missionId)
    if db_mission is None:
        raise NotFoundException(detail=f"Mission with ID {missionId} not found")
    try:
        db.delete(db_mission)
        db.commit()
        return {"message": f"Mission with ID {missionId} deleted successfully"}
