...
from database.models import NewsPost # Adjust import path if necessary
...
def deleteNewsPostByNewsPostId(
    newsPostId: int = Path(..., description="ID of the news post to delete"),
    db: Session = Depends(get_db)
) -> Dict[str, str]:
    db_news_post = db.get(NewsPost, newsPostId)
    if db_news_post is None:
        raise NotFoundException(detail=f"News post with ID {newsPostId} not found")
    try:
        db.delete(db_news_post)
        db.commit()
        return {"message": f"News post with ID {newsPostId} deleted successfully"}
