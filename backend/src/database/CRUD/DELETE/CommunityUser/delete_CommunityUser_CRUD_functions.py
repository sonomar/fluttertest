...
from database.models import CommunityUser # Adjust import path if necessary
...
def deleteCommunityUserByCommunityUserId(
    communityUserId: int = Path(..., description="ID of the CommunityUser to delete"),
    db: Session = Depends(get_db)
) -> Dict[str, str]:
    db_communityUser = db.get(CommunityUser, communityUserId)
    if db_communityUser is None:
        raise NotFoundException(detail=f"CommunityUser with ID {communityUserId} not found")
    try:
        db.delete(db_communityUser)
        db.commit()
        return {"message": f"CommunityUser with ID {communityUserId} deleted successfully"}
