...
from database.models import Collection # Adjust import path if necessary
...
def deleteCollectionByCollectionId(
    collectionId: int = Path(..., description="ID of the collection to delete"),
    db: Session = Depends(get_db)
) -> Dict[str, str]:
    db_collection = db.get(Collection, collectionId)
    if db_collection is None:
        raise NotFoundException(detail=f"Collection with ID {collectionId} not found")
    try:
        db.delete(db_collection)
        db.commit()
        return {"message": f"Collection with ID {collectionId} deleted successfully"}
