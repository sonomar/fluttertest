...
from database.models import Community # Adjust import path if necessary
...
def deleteCommunityByCommunityId(
    communityId: int = Path(..., description="ID of the community to delete"),
    db: Session = Depends(get_db)
) -> Dict[str, str]:
    db_community = db.get(Community, communityId)
    if db_community is None:
        raise NotFoundException(detail=f"Community with ID {communityId} not found")
    try:
        db.delete(db_community)
        db.commit()
        return {"message": f"Community with ID {communityId} deleted successfully"}
