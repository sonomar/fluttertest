...
from database.models import CollectibleSponsor # Adjust import path if necessary
...
def deleteCollectibleSponsorByCollectibleSponsorId(
    collectibleSponsorId: int = Path(..., description="ID of the collectible to delete"),
    db: Session = Depends(get_db)
) -> Dict[str, str]:
    db_collectibleSponsor = db.get(CollectibleSponsor, collectibleSponsorId)
    if db_collectibleSponsor is None:
        raise NotFoundException(detail=f"CollectibleSponsor with ID {collectibleSponsorId} not found")
    try:
        db.delete(db_collectibleSponsor)
        db.commit()
        return {"message": f"CollectibleSponsor with ID {collectibleSponsorId} deleted successfully"}
