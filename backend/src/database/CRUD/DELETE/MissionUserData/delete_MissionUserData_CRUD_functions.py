...
from database.models import MissionUserData # Adjust import path if necessary
...
def deleteMissionUserDataByMissionUserDataId(
    missionUserDataId: int = Path(..., description="ID of the MissionUserData record to delete"),
    db: Session = Depends(get_db)
) -> Dict[str, str]:
    db_mission_user_data = db.get(MissionUserData, missionUserDataId)
    if db_mission_user_data is None:
        raise NotFoundException(detail=f"MissionUserData record with ID {missionUserDataId} not found")
    try:
        db.delete(db_mission_user_data)
        db.commit()
        return {"message": f"MissionUserData record with ID {missionUserDataId} deleted successfully"}
